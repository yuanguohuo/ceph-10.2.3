...
void ReplicatedPG::do_op(OpRequestRef& op)
{
      ...
      (!obc->obs.exists || ((m->get_snapid() != CEPH_SNAPDIR) && obc->obs.oi.is_whiteout()))) 
  {
    ...
    dout(99) << "YuanguoDbg: ReplicatedPG::do_op, call reply_ctx with -ENOENT" << dendl;
...
void ReplicatedPG::execute_ctx(OpContext *ctx)
{
  ...
  if (op->may_write() || op->may_cache()) 
  {
    dout(99) << "YuanguoDbg: ReplicatedPG::execute_ctx, may_write or may_cache" << dendl;
    // snap
    if (!(m->has_flag(CEPH_OSD_FLAG_ENFORCE_SNAPC)) && pool.info.is_pool_snaps_mode())
    {
      // use pool's snapc
      ctx->snapc = pool.snapc;
      dout(99) << "YuanguoDbg: ReplicatedPG::execute_ctx, use pool's snapc, snapc.seq=" << ctx->snapc.seq << dendl;
    }
    else
    {
      ...
      dout(99) << "YuanguoDbg: ReplicatedPG::execute_ctx, client specified snapc, snapc.seq=" << ctx->snapc.seq << dendl;
    }
  ...
  dout(99) << "YuanguoDbg: ReplicatedPG::execute_ctx, before prepare_transaction" << dendl;

  int result = prepare_transaction(ctx);

  dout(99) << "YuanguoDbg: ReplicatedPG::execute_ctx, after prepare_transaction, result=" << result << dendl;
...
void ReplicatedPG::reply_ctx(OpContext *ctx, int r)
{
  ...
  close_op_ctx(ctx);
  dout(99) << "YuanguoDbg: ReplicatedPG::reply_ctx, released rw locks, r=" << r << dendl;
}
...
void ReplicatedPG::reply_ctx(OpContext *ctx, int r, eversion_t v, version_t uv)
{
  ...
  close_op_ctx(ctx);
  dout(99) << "YuanguoDbg: ReplicatedPG::reply_ctx, released rw locks, r=" << r << " uv=" << uv << dendl;
}
...
ReplicatedPG::OpContextUPtr ReplicatedPG::trim_object(const hobject_t &coid)
{
  ...
  if (!ctx->lock_manager.get_snaptrimmer_write(
    ...
    dout(99) << "YuanguoDbg: ReplicatedPG::trim_object, released rw locks AAA" << dendl;
    return NULL;
  }
  ...
  if (!ctx->lock_manager.get_snaptrimmer_write(
    ...
    dout(99) << "YuanguoDbg: ReplicatedPG::trim_object, released rw locks BBB" << dendl;
    return NULL;
  }
...
int ReplicatedPG::prepare_transaction(OpContext *ctx)
{
  ...
  dout(99) << "YuanguoDbg: ReplicatedPG::prepare_transaction, before do_osd_ops" << dendl;
  int result = do_osd_ops(ctx, ctx->ops);
  dout(99) << "YuanguoDbg: ReplicatedPG::prepare_transaction, after do_osd_ops, result=" << result << dendl;
  ...
  if ((ctx->delta_stats.num_bytes > 0 || ctx->delta_stats.num_objects > 0) &&  // FIXME: keys?
      (pool.info.has_flag(pg_pool_t::FLAG_FULL) || get_osdmap()->test_flag(CEPH_OSDMAP_FULL)))
  {
    MOSDOp *m = static_cast<MOSDOp*>(ctx->op->get_req());
...
void ReplicatedPG::finish_ctx(OpContext *ctx, int log_op_type, bool maintain_ssc, bool scrub_ok)
{
  ...
  if (soid.snap == CEPH_NOSNAP && maintain_ssc) 
  {
    ...
    else if (ctx->new_snapset.clones.size() && !ctx->cache_evict && (!ctx->snapset_obc || !ctx->snapset_obc->obs.exists))
    {
      ...
      ctx->log.push_back(pg_log_entry_t(pg_log_entry_t::MODIFY, snapoid, ctx->at_version, eversion_t(), 0, osd_reqid_t(), ctx->mtime));
      ...
      {
        got = ctx->lock_manager.get_write_greedy(snapoid, ctx->snapset_obc, ctx->op);
      }
      else
      {
        assert(ctx->lock_type == ObjectContext::RWState::RWEXCL);
        got = ctx->lock_manager.get_lock_type(ObjectContext::RWState::RWEXCL, snapoid, ctx->snapset_obc, ctx->op);
      }
...
void ReplicatedPG::apply_ctx_stats(OpContext *ctx, bool scrub_ok)
{
  ...
  {
    assert(cmp(soid, scrubber.start, get_sort_bitwise()) < 0 || cmp(soid, scrubber.end, get_sort_bitwise()) >= 0);
...
void ReplicatedPG::on_change(ObjectStore::Transaction *t)
{
  ...
  for (list<pair<OpRequestRef, OpContext*> >::iterator i =
    ...
    dout(99) << "YuanguoDbg: ReplicatedPG::on_change, released rw locks" << dendl;
    if (is_primary())
      requeue_op(i->first);
  }
